...
- `docker-compose.yml` — reference stack to run Traefik on an edge node.
...
1. Control Plane picks a node and writes a stack with `Host(...)`, `PathPrefix(...)` labels.
...
## Bootstrapping an edge node

```bash
docker swarm init --advertise-addr <edge_ip>
docker node update --label-add hostaffin_role=edge <node_id>
